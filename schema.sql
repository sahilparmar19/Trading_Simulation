...
-- Target Database: tradingdb (PostgreSQL)

-- 1. Drop existing tables if they exist (ordered to resolve foreign key constraints)
...
-- Reset Serial sequence for sectors to sync with hardcoded IDs
SELECT setval('sectors_sector_id_seq', 5);

-- Seed Stocks (4-5 stocks per sector)
INSERT INTO stocks (ticker, company_name, sector_id, current_price, open_price, prev_close, market_cap, pe_ratio, pb_ratio, roe, roa, total_shares, promoter_hold, inst_hold, retail_hold, is_listed, exchange) VALUES
-- Sector 1: Energy
...
-- Seed Price History for ALL stocks (covers 7D, 1M, 6M, 52W ranges)
-- Each stock has ~14 data points showing realistic price movement
...
(2, 'RELIANCE', 10, 2400.00), -- Alice holds 10 RELIANCE
(2, 'TCS', 5, 3250.00),       -- Alice holds 5 TCS
